"""
OptiRoute - Full Streamlit VRP app (Deterministic — no stochastic sensitivity)
...
# ---------------- Part 2/4 ----------------
# Streamlit UI & initial data (deterministic)
...
st.markdown("Click 'Run full pipeline' to perform clustering → routing → KPIs.")
...
# ---------------- Part 3/4 ----------------
# Stage 2: routing + KPIs (Part 3/4) — deterministic

st.subheader("5) Stage 2: Routing (per-cluster OR-Tools) and KPI computation")

if run_pipeline:
    # Use base (deterministic) demands and distances from session
    varied_demands = st.session_state['demands'].copy().astype(int)
    varied_dist = st.session_state['dist_matrix']
    ...
    st.info("Press 'Run full pipeline' to perform clustering → routing → KPI calculation.")

# ---------------- end Part 3/4 ----------------


# ---------------- Part 4/4 ----------------
# KPI Dashboard (placed BELOW the routes table)
